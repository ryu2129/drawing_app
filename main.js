// ページの読み込みが完了したらコールバック関数が呼び出される
// ※コールバック： 第二関数の無名関数（＝関数名が省略された関数）
window.addEventListener('load', () => {
  const canvas = document.querySelector('#draw-area');
  // contextを使ってcabvasに絵を描いていく
  ...
  function draw (x,y) {
    ...
    // ドラッグしながらしか絵を描くことができない
    if(!isDrag) {
      return;
    }

    // 「context.beginPath()」と「context.closePath()」を都度drawするよりも、
    // 線の描き始め(dragStart関数)と線の描き終わり(dragEnd)で1回ずつ読んだ方がより綺麗に線が描ける
